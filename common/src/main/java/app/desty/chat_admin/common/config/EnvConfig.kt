...
@SerializeConfig(mmapID = "env_config", cryptKey = "h8F!dwVkmjRkfK%#")
object EnvConfig {
    var currentEnvironment: Environment by serialLazy(Environment.Dev)

    private fun getEnvironment() =
        if (BuildConfig.desty_debug) currentEnvironment else Environment.Prod
